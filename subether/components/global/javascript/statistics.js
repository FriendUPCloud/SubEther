...
function SaveStatSetting( ele, aid )
{
	...
	{
		var j = new bajax ();
		j.openUrl ( getPath() + '?component=global&action=updatestatsetting', 'post', true );
...
function DeleteStatSetting( aid )
{
	if ( !aid ) return;
	...
	{
		var j = new bajax ();
		j.openUrl ( getPath() + '?component=global&action=deletestatsetting', 'post', true );
		j.addVar ( 'aid', aid );
		j.onload = function ()
		{
			var r = this.getResponseText ().split ( '<!--separate-->' );
