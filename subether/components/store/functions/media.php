...
if( $_POST['fid'] > 0 && ( $image = $database->fetchObjectRow( '
	...
	WHERE 
		...
		AND im.ID = \'' . $_POST['fid'] . '\' 
', false, 'components/store/functions/media.php' ) ) )
{
	$str = ''; $style = '';
	...
	if( $image->Filename )
	{
		...
		{
			$onclick = 'SwitchProductImage(\'' . $_POST['fid'] . '\',\'' . $_POST['pid'] . '\')';
		}
		...
		if ( $id && isset( $parent->access->IsAdmin ) )
		{
			...
			$str .= '<input type="file" class="file_upload_btn" id="FilesUploadBtn_' . $_POST['pid'] . $image->ID . '" name="store" onchange="fileselect( this, \'FilesUpload_' . $_POST['pid'] . $image->ID . '\' )"/>';
			
			if( $_POST['pid'] )
			{
				$str .= '<input type="hidden" name="productid" value="' . $_POST['pid'] . '">';
			}
