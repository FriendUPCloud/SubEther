...
if ( $rows = $database->fetchObjectRows( $query ) )
{
	...
	if ( isset( $_REQUEST['groupid'] ) )
	{
		foreach ( $rows as $row )
		{
			...
			$catids = ( $catids && !strstr( $catids, $row->CategoryID ) ? ( $catids . ',' . $row->CategoryID ) : $catids );
		}
	}
	else
	{
	...
	if ( $usrs && ( $events = $database->fetchObjectRows( $q = '
		SELECT 
			h.*, a.MemberID, o.OrderID, o.JobID, o.CustomerID, o.F6 AS ProjectName 
		FROM 
			SBookOrders o,
		...
		foreach ( $events as $evt )
		{
			if ( $evt->UserID > 0 )
			{
				if ( !isset( $hrs[$evt->UserID] ) )
				{
					...
					$obj->MemberID = ( isset( $pts[$evt->UserID] ) ? $pts[$evt->UserID] : $evt->MemberID );
...
if ( isset( $parent->folder->Settings ) && $parent->folder->Settings )
{
	$settings = json_obj_decode( $parent->folder->Settings );
}

//die( print_r( $settings->AccessLevels,1 ) . ' --' );
...
if ( $hrs )
{
	...
	switch ( $_REQUEST['type'] )
	{
		...
		case 'pdf':
			...
			die ( @$pdf->Output( 'Hourlist_' . date( 'Ymd' ) . '.pdf', 'I' ) );
