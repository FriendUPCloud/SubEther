...
function RefreshOrderCalendar( date, ele )
{
	if( !date || !ele ) return;
	...
	var j = new bajax ();
	j.openUrl ( getPath() + '?component=orders&function=calendar', 'post', true );
	j.addVar ( 'date', date );
	j.onload = function ()
	{
...
function CloseOrderCalendar( ele )
{
	if( !ele ) return;
