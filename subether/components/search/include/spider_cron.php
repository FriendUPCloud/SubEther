...
$i = 0; $ii = 0; $links = 0; $broken = 0; $parsed = 0;

if ( $rows = $database->fetchObjectRows ( 'SELECT * FROM SEngineLinks WHERE IsParsed = "0" AND IsBroken = "0" ORDER BY ID ASC LIMIT 100' ) )
{
	foreach ( $rows as $row )
	{
		$s = ''; $r = '';
...
print ( "\n\n" . 'Total of ' . $i . ' links got ' . $links . ' new links ' . $broken . ' is broken and ' . $parsed . ' are parsed in (' . number_format( ( $time = $time_end - $time_start ), 2, '.', '' ) . ' seconds)' . "\n" );

$database->close ();
print ( "\nDone\n" );

$log = fopen ( 'spider.log', 'a+' );
fwrite ( $log, date ( 'd/m/Y H:i:s' ) . ' - Processed - Total of ' . $i . ' links got ' . $links . ' new links ' . $broken . ' is broken and ' . $parsed . ' are parsed in (' . number_format( ( $time = $time_end - $time_start ), 2, '.', '' ) . ' seconds)' . "\n" );
fclose ( $log );
