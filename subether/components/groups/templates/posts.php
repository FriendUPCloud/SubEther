...
<div class="posts">
	...
	<?if ( $this->parent->webuser->ID == $GLOBALS[ 'webuser' ]->ID ) { ?>
	<div class="Box">
		<div class="tabs">
			<table>
				<tr>
	...
	<?}?>
	<div id="ShareContent"><? return $this->parent->content; ?></div>
	...
	<script type="text/javascript">
		...
		function addShareContent ( o )
		{
			...
			{
				var j = new bajax ();
				j.openUrl ( '<?= $this->parent->curl ?>?function=savesharemessage', 'post', true );
		...
		function loadShareMessages (  )
		{
			...
			var j = new bajax ();
			j.openUrl ( '<?= $this->parent->curl ?>?function=sharedposts', 'post', true );
			j.addVar ( 'CategoryID', <?= $this->parent->folder->CategoryID ?> );
			j.addVar ( 'UserID', <?= $this->parent->webuser->ID ?> );
			j.onload = function ()
			{
				var r = this.getResponseText ().split ( '<!--separate-->' );
				if ( r[0] == 'ok' )
