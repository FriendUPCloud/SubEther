...
statistics( $parent->module, 'broadcast' );

$root = 'subether/';
$cbase = 'subether/components/broadcast';

include_once ( $cbase . '/include/functions.php' );

// Setup resources -------------------------------------------------------------
$document->addResource ( 'stylesheet', $cbase . '/css/component.css' );
$document->addResource ( 'javascript', $cbase . '/javascript/broadcast.js' );

// Template: Module, Component, CategoryID, Status
SessionSet ( $parent->module, 'broadcast', $parent->folder->CategoryID, 'online' );
...
if ( isset( $_REQUEST[ 'action' ] ) )
{
	...
	die( 'failed action request - broadcast' );
}
...
else if ( isset( $_REQUEST[ 'function' ] ) )
{
	...
	die( 'failed function request - broadcast' );
}
else
{
	if( isset( $_REQUEST[ 'e' ] ) )
	{
		include ( $cbase . '/functions/event.php' );
	}
	else
	{
		include ( $cbase . '/functions/overview.php' );
	}
