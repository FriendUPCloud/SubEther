...
class FacebookCurlHttpClient implements FacebookHttpable
{
  ...
  public function openConnection($url, $method = 'GET', array $parameters = array())
  {
    $options = array(
      ...
      CURLOPT_POSTFIELDS     => 'email=' . urlencode( FACEBOOK_DEV_USERNAME ) . '&pass=' . urlencode( FACEBOOK_DEV_PASSWORD ) . '&login=Login',
      CURLOPT_COOKIEFILE     => '/tmp/cookies/'.session_id().'.txt',
      CURLOPT_COOKIEJAR      => '/tmp/cookies/'.session_id().'.txt',
    );
