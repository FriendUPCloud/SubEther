...
fcrypt = {
	...
	b64to16: function ( s )
	{
        ...
        for ( i = 0; i < s.length; ++i )
        {
            ...
            if ( k == 0 )
            {
                ret += ( deps ? deps.int2char( v >> 2 ) : int2char( v >> 2 ) );
                slop = v & 3;
                k = 1;
            }
            else if ( k == 1 )
            {
                ret += ( deps ? deps.int2char( (slop << 2) | (v >> 4) ) : int2char( (slop << 2) | (v >> 4) ) );
                slop = v & 0xf;
                k = 2;
            }
            else if ( k == 2 )
            {
                ret += ( deps ? deps.int2char( slop ) : int2char( slop ) );
                ret += ( deps ? deps.int2char( v >> 2 ) : int2char( v >> 2 ) );
                slop = v & 3;
                k = 3;
            }
            ...
            {
                ret += ( deps ? deps.int2char( (slop << 2) | (v >> 4) ) : int2char( (slop << 2) | (v >> 4) ) );
                ret += ( deps ? deps.int2char( v & 0xf ) : int2char( v & 0xf ) );
                k = 0;
            }
        }
        if ( k == 1 ) ret += ( deps ? deps.int2char( slop << 2 ) : int2char( slop << 2 ) );
        return ret;
    },
